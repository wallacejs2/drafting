...
const OutlookCard: React.FC<{ title: string; content: string; icon: React.ReactNode }> = ({ title, content, icon }) => (
    <div className="bg-bg-primary/50 border border-border-primary/50 rounded-lg p-4">
...
const KeyFactors: React.FC<{ modifiers: Player['projectionModifiers'] }> = ({ modifiers }) => {
    ...
    return (
        <div className="bg-bg-primary/50 border border-border-primary/50 rounded-lg p-4">
            <h2 className="text-xl font-bold text-text-primary mb-3">Key Factors & Modifiers</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <h3 className="text-lg font-bold text-accent-positive flex items-center gap-2 mb-2">
                 ...
                 <div>
                    <h3 className="text-lg font-bold text-accent-negative flex items-center gap-2 mb-2">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
const PlayerAnalyticsWorkstation: React.FC<PlayerAnalyticsWorkstationProps> = ({ player, outlook, onClose }) => {
    ...
    return (
        <div role="dialog" aria-modal="true" className="fixed inset-0 z-50">
            ...
            <div
                className={`absolute top-0 right-0 h-full w-full max-w-lg bg-bg-secondary border-l border-border-primary shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}
            >
                <header className="flex-shrink-0 flex items-center justify-between p-4 border-b border-border-primary">
                    ...
                    <button
                        ...
                        className="text-text-secondary p-1 rounded-full hover:bg-bg-primary hover:text-text-primary transition-colors"
                    >
                ...
                <main className="flex-grow overflow-y-auto p-4 md:p-6">
                    <div className="bg-bg-primary/50 border border-border-primary/50 rounded-lg p-4 mb-6">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                            <div>
                                <h2 className="text-3xl font-bold text-text-primary">{player.name}</h2>
                                <div className="flex items-center gap-4 mt-2 flex-wrap">
                                    <p className="text-lg font-semibold text-accent-primary">{player.position} - {player.team}</p>
                                    <p className="text-sm font-bold bg-border-primary px-2 py-1 rounded text-text-secondary">TIER {player.tier}</p>
                                    <p className="text-sm font-bold bg-bg-primary px-2 py-1 rounded text-text-secondary">BYE {player.byeWeek}</p>
                                </div>
                    ...
                    {!outlook ? (
                        <div className="flex flex-col items-center justify-center h-96">
                            ...
                            <p className="mt-4 text-md text-text-primary font-semibold">Analyzing {player.name}...</p>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-6 animate-fade-in">
                            <div className="bg-bg-primary/50 border border-border-primary/50 rounded-lg p-4">
                                <h2 className="text-xl font-bold text-text-primary mb-3">Season Outlook</h2>
                                <p className="text-text-secondary leading-relaxed mb-4 text-sm">{outlook.summary}</p>
                                <div className="border-t border-border-primary pt-4">
                                     <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <h3 className="text-lg font-bold text-text-primary">Final Verdict</h3>
                                    </div>
                                    <p className="text-text-secondary text-sm font-medium">{outlook.verdict}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-bg-primary/50 border border-border-primary/50 rounded-lg p-4">
                                ...
                                <p className="text-text-secondary text-sm">{outlook.expertConsensus}</p>
                            </div>
                            ...
                            <div className="space-y-4">
                                <OutlookCard title="Upside" content={outlook.upside} icon={
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-accent-positive" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>
                                } />
                                <OutlookCard title="Downside" content={outlook.downside} icon={
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-accent-negative" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6" /></svg>
                                } />
                            </div>
